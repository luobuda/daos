...
短参: -b
  长参: --block_max
  含义: 单次 update 最大块数（1 块 = 4KB）
  默认值: 256（1MB）；不支持单位后缀
  ────────────────────────────────────────
  短参: -C
  长参: --capacity
  含义: VEA 逻辑容量，支持 k/m/g/t 后缀
  默认值: 1TB
  ────────────────────────────────────────
  短参: -c
  长参: --cont_nr
  ...
  长参: --file
  含义: pmemobj 池文件路径
  默认值: /mnt/daos/vea_stress_pool
  ────────────────────────────────────────
  短参: -H
  长参: --heap
  含义: 分配器堆大小，支持 k/m/g/t 后缀
  默认值: 100MB
  ────────────────────────────────────────
  短参: -l
  长参: --load
  含义: 只加载已有池（量 vea_load 耗时），不跑负载
  默认值: 关
  ────────────────────────────────────────
  短参: -o
  长参: --obj_nr
...
# a) 本地快速冒烟：小容量小堆，30秒，指定路径避免 /mnt/daos不存在
  vea_stress -f /tmp/vea/stress.pool -C 128g -H 512m -c 4 -o 200 -b 128 -d 30

  # b) 复现某次失败 /
  固定负载：种子必须显式指定（否则每次都是time(NULL)）
  vea_stress -f /tmp/vea/stress.pool -C 128g -H 512m -c 4 -o 200 -b 128 -d 30 -s 42

  # c) 测“碎片化后 vea_load 有多慢”（VEA加载耗时是重点优化项）
  vea_stress -f /tmp/vea/stress.pool -C 128g -H 512m -c 4 -d 60      # 先造出一个碎片池
  vea_stress -f /tmp/vea/stress.pool -C 128g -H 512m -c 4 -l         # 再只做 load + query

  # d) 压大 extent 场景（-b 512 => 单次最多2MB），堆给大点
  vea_stress -f /tmp/vea/stress.pool -C 1t -H 2g -c 2 -o 500 -b 512 -d 120 -s 7
